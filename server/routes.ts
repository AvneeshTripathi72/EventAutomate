...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/resumes", resumeLimiter, jobSeekerSession, async (req, res) => {
    try {
      ...
      console.log("Received POST /api/resumes with body keys:", Object.keys(req.body));
      console.log("resumeFile exists?", !!resumeFile, "type:", typeof resumeFile);
      if (resumeFile) {
        console.log("resumeFile.data exists?", !!resumeFile.data, "length:", resumeFile.data?.length);
      }
      const fileError = validateResumeFileObject(resumeFile);
      if (fileError) {
        console.log("validateResumeFileObject failed:", fileError);
        return res.status(400).json({ error: fileError });
      }
      ...
      if (resumeFile && resumeFile.data) {
        try {
          console.log("Calling processBase64Upload...");
          finalResumeUrl = await processBase64Upload(resumeFile.data, "resumes");
          console.log("Upload successful, URL:", finalResumeUrl);
        } catch (uploadErr) {
  ...
  app.post("/api/jobseekers/register", registerLimiter, jobSeekerSession, async (req, res) => {
    try {
      ...
      const existingUser = await supabase.from("job_seekers").select("*").eq("email", validatedData.email).single().then(r => r.data ? [toCamel(r.data)] : []);
      ...
      const [jobSeeker] = await supabase.from("job_seekers").insert(toSnake({
        ...validatedData,
        password: hashedPassword,
      })).select().then(r => toCamel(r.data) || []);
  ...
  app.post("/api/jobseekers/login", loginLimiter, jobSeekerSession, async (req, res) => {
    try {
      ...
      const [jobSeeker] = await supabase.from("job_seekers").select("*").eq("email", validatedData.email).single().then(r => r.data ? [toCamel(r.data)] : []);
  ...
  app.get("/api/jobseekers/me", jobSeekerSession, async (req, res) => {
    ...
    try {
      const [jobSeeker] = await supabase.from("job_seekers").select("id, full_name, email, phone, current_position, experience_level, is_hotlisted, hotlist_notes, created_at").eq("id", req.session.jobSeeker.id).single().then(r => r.data ? [toCamel(r.data)] : []);
      if (!jobSeeker) return res.status(404).json({ error: "Profile not found" });
      res.json(jobSeeker);
    } catch (err) { console.error(err); res.status(500).json({ error: "Failed to fetch profile" }); }
  });
  ...
  app.post("/api/company/users", requireCompanyAdmin, async (req, res) => {
    try {
      ...
      const existing = await supabase.from("users").select("id").eq("username", data.username).single().then(r => r.data ? [toCamel(r.data)] : []);
      if (existing.length > 0) return res.status(409).json({ error: "Username already taken" });
      const hashed = await hashPassword(data.password);
      const [created] = await supabase.from("users").insert(toSnake({
  ...
  app.post("/api/interviews", requireAuth, async (req, res) => {
    try {
      ...
      try {
        ...
        const [app] = await supabase.from("applications").select("*").eq("id", data.applicationId).single().then(r => r.data ? [toCamel(r.data)] : []);
        if (app && app.email) {
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      ...
      const jobsContext = allJobs.slice(0, 5).map(j => `- ${j.title} at ${j.company} (${j.location})`).join("\n");
      
      const systemPrompt = `You are Agastya, the official AI recruitment assistant for Tilcons (an ATS and CRM platform).
Your primary role is to assist job seekers and employers with recruitment, hiring, our platform features, and open job listings.
STRICT RULES:
1. ONLY answer questions related to recruitment, hiring, careers, job searching, ATS/CRM features, or the Tilcons platform.
2. If the user asks about ANYTHING else (e.g. coding, general knowledge, recipes, personal advice, unrelated companies), you MUST politely decline and steer the conversation back to Tilcons and recruitment.
3. Keep responses concise and helpful.
4. Here are some currently open jobs you can mention if asked:\n${jobsContext || 'No jobs currently posted.'}`;

      const openai = getOpenAI();
      const response = await openai.chat.completions.create({
        model: process.env.AI_INTEGRATIONS_OPENAI_MODEL || "gpt-3.5-turbo",
        messages: [
      ...
      const reply = response.choices[0]?.message?.content || "I apologize, but I am unable to respond at the moment.";
      res.json({ reply });
    } catch (error) {
